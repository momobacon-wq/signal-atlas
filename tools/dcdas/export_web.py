...
def run(conn, docs: Path, repo: Path, log=print, passphrase: str = None):
    ...
    rc_by_block = {}                   # block_id -> [nDecl, nLink, nPair, nXref] for opaque macros with recovered interface pins
    ORG = {"decl": "d", "link": "l", "pair": "p", "xref": "x"}
    for r in conn.execute("""SELECT block_id,name,direction,dir_source,conn_kind,connection,var_id,tgt_block_id,tgt_pin,address,alias,description,origin
        ...
        if r[12]:
            rc_by_block.setdefault(r[0], [0, 0, 0, 0])[{"decl": 0, "link": 1, "pair": 2}.get(r[12], 3)] += 1
        if r[6] is not None:
