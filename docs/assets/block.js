...
(function () {
  ...
  /** 腳位列（方塊頁／側欄共用）：第 12 欄 org（回推腳，只在不透明方塊上）→「腳位來源」欄：明文／宣告（推）／連線（推）／配對（推）／查證（證） */
  ...
  D.page('b', async ({ route, view, signal }) => {
    ...
    if (pins.length) pinsBody = D.frag(
      nOrg ? D.h('p', { class: 'muted small', text: '「腳位來源」= 明文（組態檔內列出）／宣告（由宣告在該腳位的變數回推）／連線（由同層方塊的 L: 連線回推）／配對（同層同編號的 AI／FF_AI 方塊：IN 讀其輸出，OUT／DEVICE_STATUS 依 ai_<名>／<名>_DS 命名與 ReferencedIn 推斷）／查證（組態工具交互參照確認、人工登錄於 tools/xref_manual.csv）；回推腳共 ' + nOrg + ' 腳，方向由證據推斷、可能不完整。' }) : null,
